...
// Return a text info-box (Konva.Group)
export function info(obj = {}) {
	...
	// Reserve a margin in the Box rectangle
...
export function update(box, txt, label) {
	let obj = box.findOne('.text')
	if (obj) obj.text(txt)
	if (label) {
		obj = box.findOne('.label')
		if (obj) obj.text(label)
	}
}
