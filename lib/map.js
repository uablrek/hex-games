// SPDX-License-Identifier: CC-BY-4.0.
/*
  This is the map module for:
  https://github.com/uablrek/hex-games/

  Map-hex attributes:
  hex
  ax
  prop
  edges
  units - Set of unit references
  d - temporary during movement
  zoc - temporary during movement
  x - rdtr
  y - rdtr
  nat - rdtr
 */

import {hexToAxial} from './grid.js' 
export const hexMap = new Map()
export const axMap = new Map()

function createHex(x, y) {
	let hex, ax, h, key
	hex = {x:x,y:y}
	ax = hexToAxial(hex)
	h = {hex:hex, ax:ax}
	key = x + 1000*y
	hexMap.set(key, h)
	key = ax.q + 1000*ax.r
	axMap.set(key, h)
	return h
}

export function init(obj) {
	let mapWidth = 0
	if ('width' in obj) mapWidth = obj.width
	let mapHeight = 0
	if ('height' in obj) mapHeight = obj.height
	// The "map" is sparse, meaning that many hex'es are
	// "undefined". For the movementAxial() function to work, we must
	// define all used hexes, and they must have a .ax field. Further,
	// we want to allow lookup with both offset and axial coordinates
	// so two Map's must be defined
	for (let x = 0; x < mapWidth; x++) {
		for (let y = 0; y < mapHeight; y++) {
			createHex(x, y)
		}
	}
	if (!obj.mapProperties) return

	// Add information from "map"
	for (const hp of obj.mapProperties) {
		let h = getHex(hp.hex)
		if (!h) h = createHex(hp.hex.x, hp.hex.y)
		for (k in hp) h[k] = hp[k]
	}
}

export function getHex(hex) {
	const k = hex.x + hex.y * 1000
	return hexMap.get(k)
}
export function getAxial(ax) {
	const k = ax.q + 1000*ax.r
	return axMap.get(k)
}
export function removeUnit(u) {
	if (!u.hex) return
	let h = getHex(u.hex)
	u.hex = null
	if (h.units) h.units.delete(u)
}
export function addUnit(u, hex) {
	let h = getHex(hex)
	if (!h) return				// (off map?)
	u.hex = hex
	if (!h.units) h.units = new Set()
	h.units.add(u)
}
