...
export function snapshot() {
	...
	let seq = top
	while (seq) {
		s.push({name: seq.name, index: seq.index, next: seq.next})
...
// "from" is a sequence object, but "to" is a sequence-name
...
// This is actually a generic function, but was initially intended as
// a sequence-step help, and thus (unfortunately) stuck in the
// 'sequence' module. I might move it, but that would be an
// incompatible change.
//
// Sequences may be many and complicated (like in RDTR), so extra help
// may be provided in a text file with format:
//
// |step-name|
// A potentially long help text
// ^This is on a new line
// |seq-name/step-name|
// Another help text
// ...
let seqHelp = new Map()
export function parseSeqHelp(shelp) {
	shelp = shelp.replaceAll('\n', ' ')
	shelp = shelp.replaceAll('^', '\n')
	const items = shelp.split('|')
	for (let i = 1; i < items.length; i += 2) {
		seqHelp.set(items[i], items[i+1].trim())
	}
}
// Key may be any unique text, e.g. "seq-name/step-name"
export function getSeqHelp(key) {
	if (seqHelp.has(key)) return seqHelp.get(key)
	return ''
}
