...
/*
  ...
  {nat:'', type:'', stat:'', sz:'', lab:'', ll:'', color:'', stroke:''}
  ...
  Attributes of a unit (grep -rohE 'u\.[a-zA-Z]+' . | sort | uniq):
  nat
  color
  stroke
  type
  ...
  s - strength
  m - movement
  i - index in the "units" array
  img - Image (Konva.Group)
  selected - temporary during fromStr()
  hex - Hex of a placed unit (dynamic)
  ohex - Old hex of a moved unit (dynamic)
  col - temporary in UnitBox
  hasAttacked - temporary during combat
  allowable - rdtr
 */
import Konva from 'konva'
import {grid, box, map} from './index.js'
...
const pathSq = `M0,1h${c.rw}v${c.rh}h${-c.rw}v${-c.rh-1}`
const pathX = `M0,1l${c.rw},${c.rh}M0,${c.rh}l${c.rw},${-c.rh+2}`
const pathPz = `M${c.rw*0.3},${c.rh*0.3}` +
	  describeArc(side*0.18, side/5, 5, 180, 360) +
	  describeArc(side*0.38, side/5, 5, 0, 180) +
	  `h${-side*0.2-1}m0,10h${side*0.2+2}`
const pathCav = `M0,${c.rh+1}L${c.rw},1`
...
const pathAir = `M${c.rw*0.3},${c.rh*0.3}` + pathAirRaw
const paraPath = `M${c.rw*0.25},${c.rh*0.93}q${c.rw*0.18},${-c.rh*0.3},${c.rw*0.25},0` +
	  `q${c.rw*0.07},${-c.rh*0.3},${c.rw*0.25},0`
export const unitTypePath = {
	res: pathSq,
	inf: pathSq + pathX,
	pz: pathSq + pathPz,
...
function createUnit(obj = {}) {
	...
	// Special unit type?
	if (conf.type in unitGenerators) {
		if (unitGenerators[conf.type](conf, u, side)) {
			if (scale != 1) u.scale({x:scale,y:scale})
			return u
		}
	}
	let upath = pathSq
	...
	return addTxt(conf, u)
}
export function addTxt(conf, u) {
	if (conf.stat) {
	...
	if (conf.lbl) {
		...
		if (txt.width() < c.rw)
			...
			txt.offsetX(txt.width() - c.rw/2)
		txt.position({x:c.lx,y:c.ly})
		txt.rotate(-90)
		u.add(txt)
	}
	// Left label
...
// ----------------------------------------------------------------------
// Additional units
const unitGenerators = {
	art: ugenArt,
}
export function addUnitGenerator(type, fn) {
	unitGenerators[type] = fn
}
function ugenArt(conf, u) {
	...
	return false				// (add text items)
}
export function ugenNav(conf, u) {
...
export function fromStr(str, filterOut) {
	...
	v = str.split(',')
	let len = v.length
	for (let i = v.length; i < 5; i++) v.push('')
	nat = v[0]
	type = v[1]
	for (const u of units) {
		const def = {
		...
		for (const prop in u) if (prop in def) def[prop] = u[prop]
		if (filterOut(u)) continue
		if (v[0] != def.nat) continue
		if (v[1] != def.type) continue
		if (v[2] && v[2] != def.stat) continue
		if (v[3] && v[3] != def.sz) continue
		if (v[4] && v[4] != def.lbl) continue
		unit = u
		break
	}
...
// 'dragend' when a unit is dragged on the map
export function unitDragend(e) {
    let u = fromImg(e.target)
	delete u.ohex
	let ohex = u.hex
    let hex = grid.pixelToHex(e.target.position())
	moveTo(u, hex, tween=false)
	if (!u.hex) {
		// Can't move here! Probably off-map. Move the unit back
		moveTo(u, ohex, tween=false)
	}
}
...
export class UnitBox {
	...
	// dragend for units in the box
	static #dragend(e) {
		let u = fromImg(e.target)
		let hex = grid.pixelToHex(e.target.position())
		// TODO: check that "hex" is on the map
		map.addUnit(u, hex)
		updateShadow(hex)
		// NOTE: you *must* 'off' old callbacks!!
		e.target.off('dragend')
		e.target.off('dragstart')
		e.target.on('dragend', unitDragend)
	...
	addUnit(u, col) {
		...
		u.img.on('dragend', UnitBox.#dragend)
...
export async function init(_units, _nations={}, _scale=1.0, _images=false) {
	...
	units = _units
	nations = _nations
	unitBase.cornerRadius(c.cornerRadius)
