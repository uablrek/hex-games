...
/*
  Hex-grid functions for
  https://github.com/uablrek/hex-games

  The pattern image problem:

  A generated grid is an Image as fillPattern in a Konva.Rect. I can't
  see a way to use a Konva.Shape as fillPattern for instance, so I
  assume we *must* use an Image. An SVG image is used, so the
  dimensions can be any floating point number (S=scalable). But the
  Image dimensions are rounded to integers.
  ...
  The "fillPatternScale" in Konva can make a tiny adjustment to
  complensate for the rounding error. For example: width=86.7 would be
  rounded to 87, but a scaleX=0.995 would bring it back. So, *always*
  set:
...
// The image size is rounded to nearest integer (by "something")
...
export function patternSvg(stroke = "black") {
	let d = osize
	...
	return `data:image\/svg+xml,<svg width="${pw}" height="${ph}" viewBox="0 0 ${pw} ${ph}" xmlns="http://www.w3.org/2000/svg"><path fill="none" stroke="${stroke}" stroke-width="1" d="${p}" /></svg>`
}
...
//
//  NOTE: ALL COORDINATES ARE AXIAL!
//
// These are likely connected to the map, so you would want them to
// return map-hex-objects rather than coordinates. This is user
// dependent, so functions are defined.
export function mapFunctions(
	...
	if (_removeD) removeD = _removeD
}
// Returns a map-hex object from an axial coordinate.
// Used by most distance/movement functions, and is mandatory
let getAxial = (ax) => ax
// Returns true if movement to 'n' from 'h' over edge 'i' is blocked.
// Used by neighboursAxial(), and indirectly by movementAxial()
let blocked = (h, n, i) => false

// Used by movementAxial(). The field ".d" is used to mark
// already-visited hexes, so removeD() should clear .d fields.
let removeD = () => undefined
let getMovementCost = (h,n,i,ref) => 1

// Return a Set of map-hex objects of "N" distance or less from a hex
...
// Return a set of reachable map-hex-objects
// The map-hex-objects *MUST* have a .ax field! The function will use
// .d in the map-hex-objects to mark already-visited hexes, so
// "removeD()" must be defined by the caller.
//
// While this function works in many cases, it may be necessary to
// write a specialized function for some games
export function movementAxial(N, ax, userRef) {
	removeD()					// remove .d in all reachable map-hex-objects
	let me = getAxial(ax)
	let s = new Set()
	crawl(N, me, s, userRef)
	s.delete(me)				// exclude my own hex
	return s
}
function crawl(N, h, s, userRef) {
	if (N < 0) return
	h.d = N 					// Movement-points left
	s.add(h)
	if (N == 0) return
	// cache neighbours
	if (!h.neigh) h.neigh = neighboursAxial(h.ax)
	for (const [i, n] of h.neigh.entries()) {
		if (!n) continue		// unreachable neighbour
		if ('d' in n && n.d > N) continue
		let cost = getMovementCost(h, n, i, userRef)
		crawl(N-cost, n, s, userRef)
	}
}
